...
widgSelectBlockOptions.push("<h1>", "H1");
widgSelectBlockOptions.push("<h2>", "H2");
widgSelectBlockOptions.push("<h3>", "H3");
widgSelectBlockOptions.push("<h4>", "H4");
widgSelectBlockOptions.push("<h5>", "H5");
widgSelectBlockOptions.push("<h6>", "H6");
//widgSelectBlockOptions.push("<p>", "Para");
...
{
	if (widgAutoClean || confirm("Desea limpiar el c\u00f3digo fuente HTML de los contenidos que acaba de pegar?"))
	{
		var matchedHead = "";
		var matchedTail = "";
...
function widgToolbarAction()
{
	...
	switch (this.action)
	{
		...
		case "link":
			...
			else
			{
				if (theIframe.contentWindow.document.selection)
				{
					...
					if (theSelection == "")
					{
						alert("Seleccione el texto para realizar el vinculo.");
				...
				var theURL = prompt("Ingrese la Ruta del vinculo Ej: http://www.google.com", "http://");
		...
		case "image":
			var theImage = prompt("Ingrese la ruta de la imagen, recuerde que la imagen debe haber sido cargada porteriormente al repositorio.", "/diseno/images/");
			
			if (theImage != null && theImage != "")
			{
				var theAlt = prompt("Ingrese un texto alternativo para la imagen:", "");
