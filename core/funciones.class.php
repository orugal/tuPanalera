...
/*
	...
	*/
	...
	function agregaCarrito($idProducto,$cantidad,$pedido,$relleno)
	{
		global $db;
		$persona = (isset($_SESSION['carrito']['persona']))?$_SESSION['carrito']['persona']['idusuario']:"";
		$query  = sprintf("INSERT INTO carrito (producto_id,cantidad,id_pedido,fecha,relleno_id,usuario_id) VALUES('%s','%s','%s','%s','%s','%s')",$idProducto,$cantidad,$pedido,date("Y-m-d H:i:s"),$relleno,$persona);
		//die($query);
		$result = $db->Execute($query);
		return $result;
	}
	...
	{
		global $db;
		$query  = sprintf("INSERT INTO pedidos (codigoPedido,fecha) VALUES('%s','%s')",$this->generacodigo(),date('Y-m-d H:i:s'));
		$result = $db->Execute($query);
		return $db->Insert_ID();
	}
